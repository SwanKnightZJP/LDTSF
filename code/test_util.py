...
def save_all_cases(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, test_save_path=None, save_result=True, metric_detail=0, nms=0):
    loader = tqdm(image_list) if not metric_detail else image_list
    ith = 0
    for image_path in loader:

        img_id = image_path.split('/')[3]
...
def save_all_cases_t(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, test_save_path=None, save_result=True, metric_detail=0, nms=0):
    loader = tqdm(image_list) if not metric_detail else image_list
    ith = 0
    for image_path in loader:
...
def test_show_all_case(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
    total_metric = 0.0
    loader = tqdm(image_list) if not metric_detail else image_list
    ith = 0
    for image_path in loader:
    ...
    return avg_metric, prediction, score_map


def test_show_all_case_t(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
    total_metric = 0.0
    loader = tqdm(image_list) if not metric_detail else image_list
    ith = 0
    for image_path in loader:
...
def test_single_case_t(net, image, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image.shape
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    y1_tanh, y1, y_body, y_detail = net(test_patch)
                    # ensemble
                    y = torch.sigmoid(y1)
                    dis_to_mask = torch.sigmoid(-1500*y1_tanh)
